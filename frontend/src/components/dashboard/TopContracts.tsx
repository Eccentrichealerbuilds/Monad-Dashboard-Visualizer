...
import { CodeIcon, CopyIcon, SearchIcon, CheckIcon } from 'lucide-react';
import { getTopContracts } from '../../services/api';
...
export const TopContracts: React.FC = () => {
  const [contracts, setContracts] = useState<TopContract[]>([]);
  ...
  useEffect(() => {
    const fetchTopContracts = async () => {
      try {
        const data = await getTopContracts();
        setContracts(data.topContracts || []);
      } catch (error) {
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
      <div className="p-4 border-b border-gray-100 dark:border-gray-700">
        <h2 className="text-lg font-semibold flex items-center gap-2">
          <CodeIcon size={18} className="text-purple-500" />
          Top Contracts (by Transactions)
        </h2>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            {contracts.map(({ address, count }) => (
              <tr key={address} className="hover:bg-purple-50 dark:hover:bg-purple-900/10">
                <td className="px-4 py-3 whitespace-nowrap">
                  <div className="flex items-center gap-2">
                    <CodeIcon size={12} className="text-purple-600 dark:text-purple-400 flex-shrink-0" />
